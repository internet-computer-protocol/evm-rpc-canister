...
The Ethereum RPC canister provides a connection between the Internet Computer and the Ethereum network. For interactions that involve value transfer, such as in the context of cross-chain asset transfers, multiple Web2  providers can be queried by a client to increase the assurance of correctness of the answer. This is a decision on the security model that is left to the client.
...
**Note**: The canister was originally designed to connect to the Ethereum blockchain from the Internet Computer. However, the canister may also be used to connect to other EVM blockchains that support Ethereum's [JSON-RPC API specification](https://ethereum.org/en/developers/docs/apis/json-rpc/).
