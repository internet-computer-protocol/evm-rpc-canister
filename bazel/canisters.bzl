...
Original: https://github.com/dfinity/ic/blob/master/bazel/canisters.bzl
"""
...
wasm_rust_binary_rule = rule(
    ...
    attrs = {
        ...
        # Applied automatically in Bazel 7.x
...
def rust_canister(name, service_file, **kwargs):
    ...
    native.genrule(
        ...
        tools = ["@crate_index//:ic-wasm"],
        cmd_bash = """
        $(location @crate_index//:ic-wasm) $(location {input_wasm}) -o $@ shrink && \
        $(location @crate_index//:ic-wasm) $@ -o $@ metadata candid:service --visibility public --file $(location {service_file})
